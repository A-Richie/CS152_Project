...
public class GameLogic {
    ...
    public boolean isCheckmate() {
        boolean flag = false;
        //If player 1 is in checkmate:
        //if p1 does not have king - p2's king is still is in play
       if(!p1.containsKing()) {
            //board[2][2].getX()
           return flag;
       }
       else if(!p2.containsKing()) {
            return flag;
       }
        //If player 2 is in check:
        return flag;
    }
    ...
    public boolean movePiece(int x, int y, int newX, int newY) {
        ...
        /*
        //updates postition of king:
        if((board[y][x] instanceof King) && board[y][x].isWhite) {
            p1.updateKingPositions(y, x);
        }
        else if (board[y][x] instanceof King && (!board[y][x].isWhite)) {
            p2.updateKingPositions(y, x);
        } */
        ...
        if (board[newY][newX] != null) {
            if (board[newY][newX].isWhite == board[y][x].isWhite) return false;
                ...
                else
                {
                    ...
                    Board.updateCapture(p2.getName(), String.valueOf(board[newY][newX].getClass().getSimpleName()));
                }
